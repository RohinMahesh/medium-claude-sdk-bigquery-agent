...
@pytest.mark.parametrize(
    "session_id, messages_return, expected_status, expected_body",
    [
        (
            "missing-session",
            [],
            404,
            {"detail": "No conversation found for session_id: missing-session"},
        ),
        (
            "valid-session",
            [
...
def test_conversation_history(
    ...
    with patch(
        "medium_claude_sdk_bigquery_agent.api.router.get_session_messages",
        return_value=messages_return,
    ):
        client = make_client()
        response = client.get(
            "/api/v0/conversation-history", params={"session_id": session_id}
        )
...
def test_clean_up(project_dir_exists, expected_status, expected_status_value):
    ...
    with (
        patch(
            "medium_claude_sdk_bigquery_agent.api.router._canonicalize_path",
            return_value="/fake/path",
        ),
        patch(
            "medium_claude_sdk_bigquery_agent.api.router._find_project_dir",
            return_value=mock_project_dir if project_dir_exists else None,
        ),
        patch("medium_claude_sdk_bigquery_agent.api.router.shutil.rmtree"),
    ):
        client = make_client()
