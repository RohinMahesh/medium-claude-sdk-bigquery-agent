...
from medium_claude_sdk_bigquery_agent.adapters.bigquery_adapter import (
    BigQueryAdapter,
    create_bq_tool,
)
from medium_claude_sdk_bigquery_agent.core.ports import BigQueryPort
from medium_claude_sdk_bigquery_agent.utils.constants import TOOL_NAME


@pytest.fixture
def mock_bigquery_adapter():
    with patch(
        "medium_claude_sdk_bigquery_agent.adapters.bigquery_adapter.bigquery.Client"
    ):
...
def test_execute_query(mock_bigquery_adapter, sql, rows, expected):
    ...
    with patch(
        "medium_claude_sdk_bigquery_agent.adapters.bigquery_adapter.bigquery_poll"
    ), patch.object(
        BigQueryAdapter,
        "execute_query",
